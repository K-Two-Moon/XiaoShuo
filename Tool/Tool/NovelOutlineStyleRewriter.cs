...
2. 输出字段和层级以当前 JSON Schema 为准；原大纲若含阶段起止章节、“涉及章节”等旧字段，不要在结果中保留。顶层起始章节数保持 {{startChapter}}，结束章节数保持 {{endChapter}}，它们仅作为工具范围元数据。
3. 可以借鉴原大纲的叙事功能和因果推进方式，但不要继承固定的分章节奏；必须重新设计具体创意，整体内容相似度尽量不高于 65%。
...
7. “阶段大纲”按剧情目标、场景、冲突或因果关系的自然变化划分，并按故事顺序排列；不要给阶段、事件、设定或线索分配具体章节序号。
8. 大纲只需完整交代故事链：每个阶段的目标、事件前因后果、关键转折、人物选择、结果与下一阶段钩子。具体章节拆分、篇幅和节奏交给后续 AI 自主安排。
9. “人物脉络”写清人物目标、行动、性格或心理变化以及关系变化；“设定与线索”写清新设定、金手指、资源、伏笔和风险。
10. 所有文字字段使用简体中文。不要在结果中提及“原大纲”“改写”“相似度”或本提示词。
